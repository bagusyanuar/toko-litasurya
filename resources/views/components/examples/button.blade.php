<div
    x-data="{ loading: false }"
    x-on:{{ $bindLoading  }}-start.window="loading = true;"
    x-on:{{ $bindLoading }}-end.window="loading = false;"
>
    <button
        x-on:click="$dispatch('button-loading-start'); $wire.call('loadingProcess').then(() => {
            $dispatch('button-loading-end');
        });"
        class="flex gap-1 py-2 px-3.5 text-xs border border-brand-500 text-white bg-brand-500 rounded-[4px] hover:bg-brand-700 hover:border-brand-700 transition-all duration-200 ease-in"
    >
        <template x-if="!loading">
            <span>Submit</span>
        </template>
        <template x-if="loading">
            <div class="w-full flex items-center justify-center">
                <svg class="w-4 h-4 animate-spinner me-1 text-inherit" xmlns="http://www.w3.org/2000/svg"
                     viewBox="0 0 24 24">
                    <g>
                        <circle cx="3" cy="12" r="1.5" class="fill-current"/>
                        <circle cx="21" cy="12" r="1.5" class="fill-current"/>
                        <circle cx="12" cy="21" r="1.5" class="fill-current"/>
                        <circle cx="12" cy="3" r="1.5" class="fill-current"/>
                        <circle cx="5.64" cy="5.64" r="1.5" class="fill-current"/>
                        <circle cx="18.36" cy="18.36" r="1.5" class="fill-current"/>
                        <circle cx="5.64" cy="18.36" r="1.5" class="fill-current"/>
                        <circle cx="18.36" cy="5.64" r="1.5" class="fill-current"/>
                    </g>
                </svg>
                <span>Loading</span>
            </div>
        </template>
    </button>
</div>
