<section id="section-table-items" data-component-id="table-item">
    <div class="bg-white w-full px-6 py-4 rounded-lg shadow-md">
        <div class="w-full flex items-center justify-between mb-3">
            ...
            <div class="flex items-center gap-3">
                <x-gxui.table.dynamic.search store="itemTableStore" dispatcher="onFindAll"></x-gxui.table.dynamic.search>
                <x-gxui.button.button wire:ignore x-on:click="$store.itemFormStore.showModal()">
        ...
        <x-gxui.table.dynamic.table store="itemTableStore" dispatcher="onFindAll" pagination="true">
            <x-slot name="header">
                ...
                <x-gxui.table.dynamic.th class="flex-1 min-w-[120px]">
                    <span>Name</span>
                </x-gxui.table.dynamic.th>
                <x-gxui.table.dynamic.th contentClass="justify-end" class="w-[100px]">
                    <span>Price (Rp)</span>
                </x-gxui.table.dynamic.th>
            ...
            <x-slot name="rows">
                <x-gxui.table.dynamic.collapsible-row>
                    <x-gxui.table.dynamic.td class="w-[140px]" contentClass="justify-center">
                        <span x-text="data.category.name"></span>
                    </x-gxui.table.dynamic.td>
                    <x-gxui.table.dynamic.td class="flex-1 min-w-[120px]">
                        <div class="flex items-center gap-3">
                            <img x-data alt="product-image" class="w-10 h-10 rounded-lg border border-neutral-200"
                                x-bind:src="data.image">
                            <span x-text="data.name"></span>
                        </div>
                    </x-gxui.table.dynamic.td>
                    <x-gxui.table.dynamic.td contentClass="justify-end" class="w-[100px]">
                        <span x-text="data.retail_price?.price.toLocaleString('id-ID') ?? '-'"
                            class="cursor-pointer transition-all ease-in duration-300 hover:underline"
                            x-on:click="toggleOpen()"></span>
                    </x-gxui.table.dynamic.td>
                    ...
                    <x-slot name="collapsible">
                        <div class="w-full flex items-start border-b">
                            <x-gxui.table.dynamic.td contentClass="justify-center"
                                class="w-[140px]"></x-gxui.table.dynamic.td>
                            <x-gxui.table.dynamic.td class="flex-1 min-w-[120px]">
                                <div class="w-full">
                                            ...
                                            <template
                                                ...
                                                <div class="flex-1 flex items-center gap-2">
                                                    <x-gxui.input.text.text placeholder="PLU" parentClassName="w-full"
                                                        ...
                                                        x-bind:disabled="$store.itemTableStore.loadingPrice"></x-gxui.input.text.text>
                                                    <x-gxui.input.text.text placeholder="Price" parentClassName="w-full"
                                                        x-model="$store.itemTableStore.prices[index].prices[idx].price"
                                                        x-mask:dynamic="$money($input, ',' ,'.', 0)"
                                                        x-bind:disabled="$store.itemTableStore.loadingPrice"
                                                        x-on:input="$store.itemTableStore.prices[index].prices[idx].price = $store.itemTableStore.formatCurrency($store.itemTableStore.prices[index].prices[idx].price)"></x-gxui.input.text.text>
                                                </div>
                                            </template>
                                        </div>
                                    </template>
                                    <div class="w-full flex justify-end mt-1">
                                        <x-gxui.button.button wire:ignore
                                            x-on:click="$store.itemTableStore.updatePrice(data.id)" class="!px-6"
                                            x-bind:disabled="$store.itemTableStore.loadingPrice">
        ...
        document.addEventListener('alpine:init', () => {
            ...
            const COMPONENT_PROPS = {
                ...
                init: function() {
                    Livewire.hook('component.init', ({
                        ...
                        const componentID = document.querySelector(
                            '[data-component-id="table-item"]')?.getAttribute('wire:id');
                    ...
                    this.component.$wire.call('findAll', this.param, this.currentPage, this.perPage)
                        .then(response => {
                            ...
                            if (success) {
                                ...
                                Alpine.nextTick(() => {
                                    this
                                        ._generatePrices(); // Baru isi setelah DOM sadar kalau harganya kosong
                                });
                            } else {
                ...
                _generatePrices() {
                    ...
                    this.prices = JSON.parse(JSON.stringify(newPrices));
                },
                updatePrice(itemID) {
