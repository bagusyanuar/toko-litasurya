<section
    id="section-form-sales-team"
    data-component-id="form-sales-team"
>
    <x-gxui.modal.form
        show="$store.salesTeamFormStore.showModalForm"
    >
        <div
            ...
            <button
                type="button"
                x-on:click="$store.salesTeamFormStore.closeModal()"
        ...
        <div class="modal-body p-6">
            <x-gxui.input.text.text
                ...
                x-model="$store.salesTeamFormStore.form.username"
                x-bind:disabled="$store.salesTeamFormStore.loading"
                validatorKey="$store.salesTeamFormStore.formValidator"
            ...
            <x-gxui.input.text.text
                ...
                label="Password"
                parentClassName="mb-3"
                x-model="$store.salesTeamFormStore.form.password"
                x-bind:disabled="$store.salesTeamFormStore.loading"
                validatorKey="$store.salesTeamFormStore.formValidator"
            ...
            <x-gxui.input.text.text
                ...
                label="Name"
                parentClassName="mb-3"
                x-model="$store.salesTeamFormStore.form.name"
                x-bind:disabled="$store.salesTeamFormStore.loading"
                validatorKey="$store.salesTeamFormStore.formValidator"
                validatorField="name"
            ></x-gxui.input.text.text>
            <x-gxui.input.text.text
                ...
                label="Phone"
                parentClassName="mb-3"
                x-model="$store.salesTeamFormStore.form.phone"
                x-bind:disabled="$store.salesTeamFormStore.loading"
                validatorKey="$store.salesTeamFormStore.formValidator"
            ...
            <x-gxui.input.text.text
                ...
                label="Address"
                parentClassName="mb-3"
                x-model="$store.salesTeamFormStore.form.address"
                x-bind:disabled="$store.salesTeamFormStore.loading"
                validatorKey="$store.salesTeamFormStore.formValidator"
        ...
        <div class="modal-footer w-full flex items-center justify-end gap-2 px-4 py-3 border-t border-neutral-300">
            ...
            <x-gxui.button.button
                wire:ignore
                x-on:click="$store.salesTeamFormStore.mutate()"
                x-bind:disabled="$store.salesTeamFormStore.loading"
                class="!px-6"
            >
                <template x-if="!$store.salesTeamFormStore.loading">
                    <div class="w-full flex justify-center items-center gap-1 text-sm">
                        <span>Submit</span>
                    </div>
                </template>
...
@push('scripts')
    <script>
        document.addEventListener('alpine:init', () => {
            const INITIAL_FORM = {id: '', username: '', password: '', name: '', phone: '', address: ''};
            const STORE_PROPS = {
                component: null,
                toastStore: null,
                tableStore: null,
                ...
                init: function () {
                    Livewire.hook('component.init', ({component}) => {
                        const componentID = document.querySelector('[data-component-id="form-sales-team"]')?.getAttribute('wire:id');
                        if (component.id === componentID) {
                            this.component = component;
                            this.toastStore = Alpine.store('gxuiToastStore');
                            this.tableStore = Alpine.store('salesTeamTableStore');
                        }
                    });
                },
                formReset() {
                ...
                hydrateForm(data) {
                    ...
                    this.form.password = '';
                    this.form.name = data['sales']['name'];
                    this.form.phone = data['sales']['phone'];
                    this.form.address = data['sales']['address'];
                    this.showModalForm = true;
                }
            };
            Alpine.store('salesTeamFormStore', STORE_PROPS);
        });
    </script>
@endpush
