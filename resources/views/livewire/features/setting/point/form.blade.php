<section
    id="section-form-point"
    data-component-id="form-point"
>
    <x-gxui.modal.form
        show="$store.pointFormStore.showModalForm"
        width="30rem"
    >
        <div
            ...
            <button
                type="button"
                x-on:click="$store.pointFormStore.closeModal()"
        ...
        <div class="modal-body p-6">
            <x-gxui.input.text.text
                ...
                x-model="$store.pointFormStore.form.point"
                x-mask:dynamic="$money($input, ',')"
                x-bind:disabled="$store.pointFormStore.loading"
                validatorKey="$store.pointFormStore.formValidator"
                validatorField="point"
            ></x-gxui.input.text.text>
            <x-gxui.input.text.text
                placeholder="0"
                label="Minimum Nominal"
                parentClassName="mb-3 flex-1"
                x-model="$store.pointFormStore.form.nominal"
                x-mask:dynamic="$money($input, ',')"
                x-bind:disabled="$store.pointFormStore.loading"
                validatorKey="$store.pointFormStore.formValidator"
                validatorField="nominal"
            ></x-gxui.input.text.text>
        </div>
        <div class="modal-footer w-full flex items-center justify-end gap-2 px-4 py-3 border-t border-neutral-300">
            ...
            <x-gxui.button.button
                wire:ignore
                x-on:click="$store.pointFormStore.mutate()"
                x-bind:disabled="$store.pointFormStore.loading"
                class="!px-6"
            >
                <template x-if="!$store.pointFormStore.loading">
                    <div class="w-full flex justify-center items-center gap-1 text-sm">
                        <span>Submit</span>
                    </div>
                </template>
...
@push('scripts')
    <script>
        document.addEventListener('alpine:init', () => {
            const INITIAL_FORM = {id: '', point: 0, nominal: 0};
            const STORE_PROPS = {
                component: null,
                toastStore: null,
                tableStore: null,
                ...
                init: function () {
                    Livewire.hook('component.init', ({component}) => {
                        const componentID = document.querySelector('[data-component-id="form-point"]')?.getAttribute('wire:id');
                        if (component.id === componentID) {
                            this.component = component;
                            this.toastStore = Alpine.store('gxuiToastStore');
                            this.tableStore = Alpine.store('pointTableStore');
                        }
                    });
                },
                formReset() {
                ...
                hydrateForm(data) {
                    ...
                    this.form.nominal = data['nominal'].toLocaleString('id-ID');
                    this.showModalForm = true;
                }
            };
            Alpine.store('pointFormStore', STORE_PROPS);
        });
    </script>
@endpush
