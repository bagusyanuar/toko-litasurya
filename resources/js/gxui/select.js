document.addEventListener('alpine:init', () => {
    ...
    Alpine.bind('gxuiSelect2Bind', () => ({
        'x-data': () => ({
            ...
            _initStyle() {
                this.element.next('.select2-container').addClass('!w-full');
                let select2Class = 'w-full !text-xs !flex !items-center !px-[0.525rem] !py-[0.45rem] !h-[2.125rem] text-neutral-700 rounded-[4px] border !border-neutral-300 outline-none focus:outline-none focus:ring-0 !focus:border-neutral-500 transition duration-300 ease-in-out';
                this.element.next('.select2-container').find('.select2-selection--single').addClass(select2Class);
                this.element.next('.select2-container').find('.select2-selection--single').find('.select2-selection__rendered').addClass('!px-[0] !leading-[2] !text-xs');
                this.element.next('.select2-container').find('.select2-selection--single').find('.select2-selection__arrow').addClass('!top-1/2 !h-[0]');
            }
        }),
    }));
